...
# fresh (header + step tracker + ONE pulsing "PRESS RETURN" action) — the
# operator never sees a scrollback wall, and the recording only ever shows
# the current step.
...
page_top() { # $1 = current step 1..4 (0 = welcome page, no tracker)
  ...
  say ""
  bold " ======================================================================"
  bold "   wikiTaTa \"Test Your Agent\" - guided run wizard"
  ...
  if [ "$1" -gt 0 ]; then
    ...
    for name in "stills camera (one photo every 30s)" \
      ...
      if [ "$i" -lt "$1" ]; then mark=" DONE "
      elif [ "$i" -eq "$1" ]; then mark=" NOW> "
      else mark="      "; fi
      if [ "$i" -eq "$1" ]; then bold "   STEP $i [$mark] $name"
      else say "   STEP $i [$mark] $name"; fi
      i=$((i+1))
    done
    say ""
  fi
...
press_return() { # $1 pulses (~1s bold <-> reverse) until Return; $2.. print below
  local first="$1"; shift
  ...
  say ""
  ( while :; do
      printf '\033[%d;1H\033[2K\033[1;7m %s\033[0m' "$prow" "$first"; sleep 0.9
  ...
  printf '\033[%d;1H\033[2K\033[1m %s\033[0m' "$prow" "$first"
  printf '\033[%d;1H' "$ROW"
}
...
# ---- PAGE: welcome ----
page_top 0
say "   Welcome to the wikiTaTa \"Test Your Agent\" testing suite."
say ""
...
say "   exactly ONE thing: when the pulsing line says PRESS RETURN,"
say "   you press the Return key. That is it."
say ""
...
page_top 2
say "   STEP 1 needed nothing from you - the stills camera is already"
say "   rolling (one photo of the screen every 30 seconds)."
...
page_top 3
say "   STEP 2 DONE - the screen recording is LIVE (see the clock"
say "   window, bottom right)."
...
page_top 4
say "   STEP 3 DONE - the guide is up (top right); your instruction is"
say "   ON THE CLIPBOARD."
say ""
